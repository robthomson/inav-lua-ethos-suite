...
local telemetry
local currentMode = inavsuite.flightmode.current or "preflight"
...
local function getStickLoadFactor()
    local rx = inavsuite.session.rx.values
...
local function applySagCompensation(voltage)
    ...
    local multiplier = inavsuite.session.modelPreferences and inavsuite.session.modelPreferences.battery and inavsuite.session.modelPreferences.battery.sag_multiplier or 0.7
...
local function fuelPercentageCalcByVoltage(voltage, cellCount)
    local bc = inavsuite.session.batteryConfig
...
local function smartFuelCalc()
    if not telemetry then
        telemetry = inavsuite.tasks.telemetry
    end

    if not inavsuite.session.isConnected or not inavsuite.session.batteryConfig then
        resetVoltageTracking()
        return nil
    end

    -- make sure we reset the method if the sensor mode changes
    if inavsuite.session.modelPreferences and inavsuite.session.modelPreferences.battery and inavsuite.session.modelPreferences.battery.calc_local then
        if lastSensorMode ~= inavsuite.session.modelPreferences.battery.calc_local then
            resetVoltageTracking()
            lastSensorMode = inavsuite.session.modelPreferences.battery.calc_local
        end
    end
    ...
    if currentMode ~= lastMode then
        inavsuite.utils.log("Flight mode changed – resetting voltage state", "info")
        resetVoltageTracking()
    ...
    if not voltageStabilised then
        if isVoltageStable() then
            inavsuite.utils.log("Voltage stabilized at: " .. voltage, "info")
            voltageStabilised = true
        else
            inavsuite.utils.log("Waiting for voltage to stabilize...", "info")
            return nil
        end
    end

    if #lastVoltages >= 1 and inavsuite.flightmode.current == "preflight" then
        local prev = lastVoltages[#lastVoltages - 1]
        if voltage > prev + voltageThreshold then
            inavsuite.utils.log("Voltage increased after stabilization – resetting...", "info")
            resetVoltageTracking()
            stabilizeNotBefore = os.clock() + preStabiliseDelay
            return nil
        end
    ...
    local now = os.clock()
    if (inavsuite.flightmode.current == "inflight" or inavsuite.flightmode.current == "postflight")
