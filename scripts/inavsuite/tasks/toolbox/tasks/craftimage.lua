...
function craftimage.wakeup()
    if inavsuite.session.toolbox.craftimage ~= nil then
        -- prevent multiple image loads
        return
    end

    local craftName = inavsuite.session.craftName
    --local modelID = inavsuite.session.modelID
    ...
    bitmapPtr = inavsuite.utils.loadImage(image1, image2, image3, image4)
    ...
    if not bitmapPtr then
        bitmapPtr = inavsuite.utils.loadImage(default_image)
    end

    inavsuite.session.toolbox.craftimage = bitmapPtr
end
