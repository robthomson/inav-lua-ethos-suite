...
--[[
    ...
    Dependencies:
        inavsuite.utils.log (function): Logs messages for debugging purposes.
        inavsuite.tasks.msp.protocol.maxTxBufferSize (number): The maximum size of the transmission buffer.
        inavsuite.tasks.msp.protocol.mspSend (function): Sends the MSP payload.
...
local function mspProcessTxQ()
    ...
    inavsuite.utils.log("Sending mspTxBuf size " .. tostring(#mspTxBuf) .. " at Idx " .. tostring(mspTxIdx) .. " for cmd: " .. tostring(mspLastReq),"debug")
    ...
    local i = 2
    while (i <= inavsuite.tasks.msp.protocol.maxTxBufferSize) and mspTxIdx <= #mspTxBuf do
    ...
    if i <= inavsuite.tasks.msp.protocol.maxTxBufferSize then
        payload[i] = mspTxCRC
        for j = i + 1, inavsuite.tasks.msp.protocol.maxTxBufferSize do payload[j] = 0 end
        mspTxBuf = {}
        mspTxIdx = 1
        mspTxCRC = 0
        inavsuite.tasks.msp.protocol.mspSend(payload)
        return false
    end
    inavsuite.tasks.msp.protocol.mspSend(payload)
    return true
...
local function mspSendRequest(cmd, payload)
    if not cmd or type(payload) ~= "table" then
        inavsuite.utils.log("Invalid command or payload","debug")
        return nil
    end
    if #mspTxBuf ~= 0 then
        inavsuite.utils.log("Existing mspTxBuf still sending, failed to send cmd: " .. tostring(cmd),"debug")
        return nil
    end
    mspTxBuf[1] = #payload
    mspTxBuf[2] = cmd & 0xFF
...
local function mspReceivedReply(payload)
    ...
    while (idx <= inavsuite.tasks.msp.protocol.maxRxBufferSize) and (#mspRxBuf < mspRxSize) do
        mspRxBuf[#mspRxBuf + 1] = payload[idx]
        local value = tonumber(payload[idx]) 
        if value then
            mspRxCRC = mspRxCRC ~ value
        else
            inavsuite.utils.log("Non-numeric value at payload index " .. idx,"debug")
        end
        idx = idx + 1
    end

    if idx > inavsuite.tasks.msp.protocol.maxRxBufferSize then
        mspRemoteSeq = seq
        return false
    end
    ...
    if mspRxCRC ~= payload[idx] and version == 0 then
        inavsuite.utils.log("Payload checksum incorrect, message failed!","debug")
        return nil
    end
    return true
end
...
local function mspPollReply()
    ...
    while os.clock() - startTime < 0.05 do
        local mspData = inavsuite.tasks.msp.protocol.mspPoll()
