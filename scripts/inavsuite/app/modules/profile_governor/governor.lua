...
local function openPage(pidx, title, script)
    ...
    if inavsuite.app.gfx_buttons["profile_governor"] == nil then inavsuite.app.gfx_buttons["profile_governor"] = {} end
    if inavsuite.preferences.menulastselected["profile_governor"] == nil then inavsuite.preferences.menulastselected["profile_governor"] = 1 end
    ...
    for pidx, pvalue in ipairs(S_PAGES) do
        ...
        if inavsuite.preferences.general.iconsize ~= 0 then
            if inavsuite.app.gfx_buttons["profile_governor"][pidx] == nil then inavsuite.app.gfx_buttons["profile_governor"][pidx] = lcd.loadMask("app/modules/governor/gfx/" .. pvalue.image) end
        else
            inavsuite.app.gfx_buttons["profile_governor"][pidx] = nil
        end

        inavsuite.app.formFields[pidx] = form.addButton(line, {x = bx, y = y, w = buttonW, h = buttonH}, {
            text = pvalue.name,
            icon = inavsuite.app.gfx_buttons["profile_governor"][pidx],
            options = FONT_S,
            paint = function()
            end,
            press = function()
                inavsuite.preferences.menulastselected["profile_governor"] = pidx
                inavsuite.app.ui.progressDisplay()
                local name = i18n("app.modules.governor.name") .. " / " .. pvalue.name
                inavsuite.app.ui.openPage(pidx, name, "profile_governor/tools/" .. pvalue.script)
            end
        })

        if pvalue.disabled == true or  inavsuite.session.governorMode == 0 then inavsuite.app.formFields[pidx]:enable(false) end

        local currState = (inavsuite.session.isConnected and inavsuite.session.mcu_id) and true or false
            
        if inavsuite.preferences.menulastselected["profile_governor"] == pidx then inavsuite.app.formFields[pidx]:focus() end
