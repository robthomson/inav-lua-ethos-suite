...
inavsuite.session.dialImageCache = inavsuite.session.dialImageCache or {}
local function loadDialPanelCached(dialId)
    local key = tostring(dialId or "panel1")
    if not inavsuite.session.dialImageCache[key] then
        local panelPath = resolveDialAsset(dialId, "widgets/dashboard/gfx/dials") or "widgets/dashboard/gfx/dials/panel1.png"
        inavsuite.session.dialImageCache[key] = inavsuite.utils.loadImage(panelPath)
    end
    return inavsuite.session.dialImageCache[key]
end
