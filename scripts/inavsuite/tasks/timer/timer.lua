...
function timer.reset()
    --inavsuite.utils.log("Resetting flight timers", "info")
    lastFlightMode = nil

    local timerSession = {}
    inavsuite.session.timer = timerSession
    inavsuite.session.flightCounted = false

    timerSession.baseLifetime = tonumber(
        inavsuite.ini.getvalue(inavsuite.session.modelPreferences, "general", "totalflighttime")
    ) or 0
...
function timer.save()
    local prefs = inavsuite.session.modelPreferences
    local prefsFile = inavsuite.session.modelPreferencesFile

    if not prefsFile then
        inavsuite.utils.log("No model preferences file set, cannot save flight timers", "info")
        return 
    end

    inavsuite.utils.log("Saving flight timers to INI: " .. prefsFile, "info")

    if prefs then
        inavsuite.ini.setvalue(prefs, "general", "totalflighttime", inavsuite.session.timer.baseLifetime or 0)
        inavsuite.ini.setvalue(prefs, "general", "lastflighttime", inavsuite.session.timer.session or 0)
        inavsuite.ini.save_ini_file(prefsFile, prefs)
    end    
end
...
local function finalizeFlightSegment(now)
    local timerSession = inavsuite.session.timer
    local prefs = inavsuite.session.modelPreferences
    ...
    if timerSession.baseLifetime == nil then
        timerSession.baseLifetime = tonumber(
            inavsuite.ini.getvalue(prefs, "general", "totalflighttime")
        ) or 0
    end
    ...
    if prefs then
        inavsuite.ini.setvalue(prefs, "general", "totalflighttime", timerSession.baseLifetime)
    end
    timer.save()
end
...
--
-- Dependencies:
--   - Relies on `inavsuite.session` for session state.
--   - Uses `inavsuite.ini` for reading and writing model preferences.
--   - Calls `finalizeFlightSegment(now)` when appropriate.
function timer.wakeup()

    -- Yield if busy doing onConnect
    if inavsuite.tasks and inavsuite.tasks.onconnect and inavsuite.tasks.onconnect.active and inavsuite.tasks.onconnect.active() then
        return
    end
    ...
    local timerSession = inavsuite.session.timer
    local prefs = inavsuite.session.modelPreferences
    local flightMode = inavsuite.flightmode.current
    ...
    if flightMode == "inflight" then
        ...
        if timerSession.live >= 25 and not inavsuite.session.flightCounted then
            inavsuite.session.flightCounted = true

            if prefs and inavsuite.ini.section_exists(prefs, "general") then
                local count = inavsuite.ini.getvalue(prefs, "general", "flightcount") or 0
                inavsuite.ini.setvalue(prefs, "general", "flightcount", count + 1)
                inavsuite.ini.save_ini_file(inavsuite.session.modelPreferencesFile, prefs)
            end
        end
