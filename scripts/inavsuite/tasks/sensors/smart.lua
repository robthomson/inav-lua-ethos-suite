--[[
 * Smart sensor aggregator for Inav
 *
...
local smartfuel = assert(inavsuite.compiler.loadfile("tasks/sensors/lib/smartfuel.lua"))()
local smartfuelvoltage = assert(inavsuite.compiler.loadfile("tasks/sensors/lib/smartfuelvoltage.lua"))()
...
local function calculateFuel()
    ...
    if inavsuite.session.modelPreferences and inavsuite.session.modelPreferences.battery and inavsuite.session.modelPreferences.battery.calc_local then
         if inavsuite.session.modelPreferences.battery.calc_local == 1 then
...
local function calculateConsumption()
            ...
            if inavsuite.session.modelPreferences and inavsuite.session.modelPreferences.battery and inavsuite.session.modelPreferences.battery.calc_local then
                if inavsuite.session.modelPreferences.battery.calc_local == 1 then
                    local capacity = (inavsuite.session.batteryConfig and inavsuite.session.batteryConfig.batteryCapacity) or 1000 -- Default to 1000mAh if not set
                    local smartfuelPct = inavsuite.tasks.telemetry.getSensor("smartfuel")
                    local warningPercentage = (inavsuite.session.batteryConfig and inavsuite.session.batteryConfig.consumptionWarningPercentage) or 30
                    ...
                    return inavsuite.tasks.telemetry.getSensor("consumption") or 0
                end
            else
...
local function createOrUpdateSensor(appId, fieldMeta, value)
    ...
    local currentModule = inavsuite.session.telemetrySensor and inavsuite.session.telemetrySensor:module()
    ...
    if not sensorCache[appId] and not negativeCache[appId] then
        ...
        if existingSensor then
            ...
            sensor:appId(appId)
            sensor:physId(0)
            sensor:module(inavsuite.session.telemetrySensor:module())
...
function smart.wakeup()
    ...
    if firstWakeup then
        log = inavsuite.utils.log
        tasks = inavsuite.tasks
        firstWakeup = false
    end

    -- If telemetry is inactive, clear caches & bail
    if not (inavsuite.session.telemetryState and inavsuite.session.telemetrySensor) then
        sensorCache = {}
        negativeCache = {}
        lastValue = {}
        return
