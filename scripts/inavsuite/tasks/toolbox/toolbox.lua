...
for _, name in ipairs(taskNames) do
    toolbox[name] = assert(inavsuite.compiler.loadfile("tasks/toolbox/tasks/" .. name .. ".lua"))(inavsuite.config)
    table.insert(wakeupHandlers, function() toolbox[name].wakeup() end)
end

function toolbox.wakeup()

    if inavsuite.session.toolbox == nil then
        return
    end
    local currentTime = os.clock()

    if inavsuite.session.isConnected and inavsuite.session.telemetryState then
