...
local userpref   = inavsuite.preferences
...
local function smartfuelCallout(value)
    ...
    if value <= 0 then
        ...
        if not lastLowFuelAnnounced then
            inavsuite.utils.playFile("status", "alerts/lowfuel.wav")
    ...
    if lastSmartfuelAnnounced == nil then
        inavsuite.utils.playFile("status", "alerts/fuel.wav")
...
local eventTable = {
    {
        ...
        event = function(value)
            ...
            if filename then
                inavsuite.utils.playFile("events", "alerts/" .. filename)
            end
        end
    },
    {
        ...
        event = function(value, interval, window)
            ...
            if avgVoltage < warnVoltage and shouldAlert(key, interval) then
                inavsuite.utils.playFile("events", "alerts/lowvoltage.wav")
    ...
    },
    {
        ...
        event = function(value, interval, window)
            ...
            if avgTemp >= escalertvalue and shouldAlert(key, interval) then
                inavsuite.utils.playFile("events", "alerts/esctemp.wav")
                system.playHaptic(". . . .")
                registerAlert(key, interval)
    ...
    },
    {
        ...
        event = function(value, interval, window)
            ...
            local batprefs = (inavsuite.session.modelPreferences and inavsuite.session.modelPreferences.battery) or {}
            ...
            if alert_type == 1 then
                if avgBEC < becalertvalue and shouldAlert(key, interval) then
                    inavsuite.utils.playFile("events", "alerts/becvolt.wav")
                    system.playHaptic(". . . .")
                    registerAlert(key, interval)
            ...
            elseif alert_type == 2 then
                if avgBEC < rxalertvalue and shouldAlert(key, interval) then
                    inavsuite.utils.playFile("events", "alerts/rxvolt.wav")
                    system.playHaptic(". . . .")
                    registerAlert(key, interval)
    ...
    },
    {
        ...
        event = function(value)
            local key = "governor"
            if value == lastValues[key] then return end
            if not inavsuite.session.isArmed or inavsuite.session.governorMode == 0 then return end
            local governorMap = {
            ...
            if filename then inavsuite.utils.playFile("events", "gov/" .. filename) end
        end
    },
    {
        sensor = "pid_profile",
        ...
        event = function(value)
            ...
            if value == lastValues[key] then return end
            inavsuite.utils.playFile("events", "alerts/profile.wav")
    ...
    },
    {
        ...
        event = function(value)
            ...
            if value == lastValues[key] then return end
            inavsuite.utils.playFile("events", "alerts/rates.wav")
            system.playNumber(math.floor(value))
        end
    }
}
...
function telemetry.wakeup()
    ...
    for _, item in ipairs(eventTable) do
        ...
        local source = inavsuite.tasks.telemetry.getSensorSource(key)
