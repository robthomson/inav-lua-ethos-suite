local folder = "flrtr"
local ESC = assert(inavsuite.compiler.loadfile("app/modules/esc_tools/mfg/" .. folder .. "/init.lua"))()
...
if inavsuite.session.escDetails and inavsuite.session.escDetails.model then
    -- known models strings are
    -- FLYROTOR 280A
    -- FLYROTOR 150A

    -- note.  if you change the order of items in mspapi above - this will need to be updated
    if string.find(inavsuite.session.escDetails.model, "FLYROTOR 150A") then
