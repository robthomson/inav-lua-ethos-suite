...
local function checkLogdirExists()
        ...
        os.mkdir("LOGS:/inavsuite/telemetry")
        os.mkdir("LOGS:/inavsuite/telemetry/" .. inavsuite.session.mcu_id)
end
...
function logging.writeLogs(forcewrite)
    ...
    if #log_queue > 0 and logFileName then
        local filePath = "LOGS:inavsuite/telemetry/" .. inavsuite.session.mcu_id .. "/" .. logFileName
...
function logging.getLogHeader()
    ...
    return "time, " .. inavsuite.utils.joinTableItems(names, ", ")
end
...
function logging.getLogLine()
    ...
    local ts = os.time()
    return ts .. ", " .. inavsuite.utils.joinTableItems(values, ", ")
end
...
function logging.flushLogs()
    if logFileName or logHeader then
        inavsuite.utils.log("Flushing logs - " .. tostring(logFileName), "info")
...
function logging.wakeup()
    ...
    if inavsuite.utils.inFlight() then
        if not logFileName then
            logFileName = generateLogFilename()
            inavsuite.utils.log("Logging triggered by inFlight() - " .. logFileName, "info")

            local iniName = "LOGS:inavsuite/telemetry/" .. inavsuite.session.mcu_id .. "/logs.ini"
            local iniData = inavsuite.ini.load_ini_file(iniName) or {}
            if not iniData.model then
                iniData.model = {}
            end
            iniData.model.name = inavsuite.session.craftName or model.name() or "Unknown"
            inavsuite.ini.save_ini_file(iniName, iniData)
        end
