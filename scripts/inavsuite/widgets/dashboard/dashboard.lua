...
function dashboard.renderLayout(widget, config)
    ...
    if layout.showstats or inavsuite.preferences.developer.overlaystats then
        ...
        local cpuUsage = inavsuite.session and inavsuite.session.cpuload or 0
        local ramUsage = inavsuite.session and inavsuite.session.freeram or 0

        lcd.font(FONT_S)

        local cpuText = "CPU: " .. inavsuite.utils.round(cpuUsage, 0) .. "%"
        local ramText = "RAM: " .. inavsuite.utils.round(ramUsage, 0) .. "kB"
...
function dashboard.wakeup(widget)
    ...
    if inavsuite.app and inavsuite.app.triggers.mspBusy and not (inavsuite.session and inavsuite.session.isConnected) then return end
    ...
    if not inavsuite.session.isConnected then
