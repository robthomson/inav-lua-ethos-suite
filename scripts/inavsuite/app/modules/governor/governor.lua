...
local function openPage(pidx, title, script)
    ...
    if inavsuite.app.gfx_buttons["governor"] == nil then inavsuite.app.gfx_buttons["governor"] = {} end
    if inavsuite.preferences.menulastselected["governor"] == nil then inavsuite.preferences.menulastselected["governor"] = 1 end
    ...
    for pidx, pvalue in ipairs(S_PAGES) do
        ...
        if inavsuite.preferences.general.iconsize ~= 0 then
            if inavsuite.app.gfx_buttons["governor"][pidx] == nil then inavsuite.app.gfx_buttons["governor"][pidx] = lcd.loadMask("app/modules/governor/gfx/" .. pvalue.image) end
        else
            inavsuite.app.gfx_buttons["governor"][pidx] = nil
        end

        inavsuite.app.formFields[pidx] = form.addButton(line, {x = bx, y = y, w = buttonW, h = buttonH}, {
            text = pvalue.name,
            icon = inavsuite.app.gfx_buttons["governor"][pidx],
            options = FONT_S,
            paint = function()
            end,
            press = function()
                inavsuite.preferences.menulastselected["governor"] = pidx
                inavsuite.app.ui.progressDisplay()
                local name = i18n("app.modules.governor.name") .. " / " .. pvalue.name
                inavsuite.app.ui.openPage(pidx, name, "governor/tools/" .. pvalue.script)
            end
        })
        ...
        if inavsuite.preferences.menulastselected["governor"] == pidx then inavsuite.app.formFields[pidx]:focus() end
