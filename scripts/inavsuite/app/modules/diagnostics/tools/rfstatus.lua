...
-- Indices into inavsuite.app.formFields (intentionally 0-based)
local IDX_CPULOAD     = 0
local IDX_FREERAM     = 1
local IDX_BG_TASK     = 2
...
local function addStatusLine(captionKey, initialText)
  inavsuite.app.formLines[inavsuite.app.formLineCnt] = form.addLine(i18n(captionKey))
  inavsuite.app.formFields[inavsuite.app.formFieldCount] = form.addStaticText(
...
local function openPage(pidx, title, script)
  ...
  -- CPU Load %
  addStatusLine("CPU Load", string.format("%.1f%%", inavsuite.session.cpuload or 0))

  -- Free RAM
  addStatusLine("Free RAM", string.format("%.1f kB", inavsuite.session.freeram or 0))


  -- Background Task status
  addStatusLine(
    "app.modules.rfstatus.bgtask",
    inavsuite.tasks.active() and i18n("app.modules.rfstatus.ok") or i18n("app.modules.rfstatus.error")
  )
...
local function wakeup()
  ...
  do
    ...
    if field then
      field:value(string.format("%.1f%%", inavsuite.session.cpuload or 0))
    end
  end
  ...
  do
    ...
    if field then
      field:value(string.format("%.1f kB", inavsuite.utils.round(inavsuite.session.freeram or 0, 1)))
    end
  end
  ...
  do
    ...
    if field then
      local sensors = inavsuite.tasks
                    and inavsuite.tasks.telemetry
                    and inavsuite.tasks.telemetry.validateSensors(false)
                    or false
