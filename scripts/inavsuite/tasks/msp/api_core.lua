...
function core.clearFileExistsCache()
...
local function parseMSPChunk(buf, structure, state)
    ...
    while state.index <= #structure and processedFields < 5 do  -- Process 5 fields per wakeup
        ...
        if field.apiVersion and inavsuite.utils.apiVersionCompare("<", field.apiVersion) then
            goto continue
        end
...
function core.calculateMinBytes(structure)
    ...
    for _, param in ipairs(structure) do
        ...
        if not param.apiVersion or inavsuite.utils.apiVersionCompare(">=", param.apiVersion) then
            insert_param = true
        end
...
function core.buildWritePayload(apiname, payload, api_structure, noDelta)
    ...
    local positionmap = inavsuite.app.Page.apidata and inavsuite.app.Page.apidata.positionmap[apiname]
    local receivedBytes = inavsuite.app.Page.apidata and inavsuite.app.Page.apidata.receivedBytes[apiname]
    local receivedBytesCount = inavsuite.app.Page.apidata and inavsuite.app.Page.apidata.receivedBytesCount[apiname]
...
function core.buildDeltaPayload(apiname, payload, api_structure, positionmap, receivedBytes, receivedBytesCount)
    ...
    for idx, formField in ipairs(inavsuite.app.formFields) do
        local pageField = inavsuite.app.Page.fields[idx]
    ...
    if inavsuite.app.Page and inavsuite.app.Page.apidata then
        for _, field in ipairs(inavsuite.app.Page.apidata.formdata.fields) do
            ...
            if field.api and not field.apikey then
                ...
                --- that means checking inavsuite.app.Page.apidata.api for the index of mspapi
...
function core.buildFullPayload(apiname, payload, api_structure)
    ...
    for _, field_def in ipairs(api_structure) do
        ...
        if not actual_field and field_def.decimals then
            scale = scale / inavsuite.app.utils.decimalInc(field_def.decimals)
        end
        value = math.floor(value * scale + 0.5)
...
function core.prepareStructureData(structure)
    ...
    for _, param in ipairs(structure) do
        if param.apiVersion and inavsuite.utils.apiVersionCompare("<", param.apiVersion) then
            goto continue
        end
