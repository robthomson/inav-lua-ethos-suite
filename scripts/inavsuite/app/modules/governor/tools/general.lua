...
local apidata = {
        ...
        formdata = {
            ...
            {t = i18n("app.modules.profile_governor.idle_throttle"),  mspapi = 1, apikey = "governor_idle_throttle", enablefunction = function() return (inavsuite.session.governorMode >=1 ) end},
            {t = i18n("app.modules.profile_governor.auto_throttle"),  mspapi = 1, apikey = "governor_auto_throttle", enablefunction = function() return (inavsuite.session.governorMode >=1 ) end},
...
local function disableFields()
    for i, _ in ipairs(inavsuite.app.formFields) do

        if type(inavsuite.app.formFields[i]) == "userdata" then
            if i ~= 1 and inavsuite.app.formFields[i] then
...
local function enableFields()
    for i, _ in ipairs(inavsuite.app.formFields) do
        if type(inavsuite.app.formFields[i]) == "userdata" then
            if i ~= 1 and inavsuite.app.formFields[i] then
               inavsuite.app.formFields[i]:enable(true)
            end
        end
    end
end
...
local function setGovernorMode(self)
    local currentIndex = math.floor(inavsuite.app.Page.fields[1].value)
...
local function postSave(self)
    ...
    if inavsuite.session.governorMode ~= inavsuite.app.Page.fields[1].value then
        inavsuite.session.governorMode = inavsuite.app.Page.fields[1].value
        inavsuite.utils.log("Governor mode: " .. inavsuite.session.governorMode,"info")
    end
    return payload
end
