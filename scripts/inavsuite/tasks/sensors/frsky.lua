...
local function telemetryActive()
  return inavsuite and inavsuite.session and inavsuite.session.telemetryState == true
end

-- Detect changes to MSP-provided whitelist
local function slotsFingerprint()
  local slots = inavsuite and inavsuite.session and inavsuite.session.telemetryConfig 
...
local function getSidList()
  local mod = inavsuite and inavsuite.tasks and inavsuite.tasks.sensors
...
----------------------------------------------------------------------
-- Public API: set Inav IDs we expect (e.g., {0,1,5,10})
...
function frsky.setFblSensors(fblIds)
  ...
  -- 1) Mark enabled S.Port appIds from Inav ids
  ...
  if inavsuite and inavsuite.tasks and inavsuite.tasks.sensors then
    inavsuite.tasks.sensors.sid = nil
  end
  collectgarbage("collect")
end

-- Default stub (until caller provides the real profile list)
frsky.setFblSensors(inavsuite.session.telemetryConfig)

----------------------------------------------------------------------
-- Helpers: create, rename (same flow; gated by tiny maps)
----------------------------------------------------------------------
local function createSensor(physId, primId, appId, frameValue)
  if inavsuite.session.apiVersion == nil then return "skip" end
  ...
  if frsky.createSensorCache[appId] == nil then
    ...
    if frsky.createSensorCache[appId] == nil then
      ...
      s:appId(appId)
      s:physId(physId)
      s:module(inavsuite.session.telemetrySensor:module())
      ...
      if v.extras then
        for _, e in ipairs(v.extras) do
          ...
          if not existing then
            ...
            sExtra:module(inavsuite.session.telemetrySensor:module())
...
local function telemetryPop()
  if not inavsuite.tasks.msp.sensorTlm then return false end

  local frame = inavsuite.tasks.msp.sensorTlm:popFrame()
...
function frsky.wakeup()
  ...
  if not inavsuite.session.telemetryState or not inavsuite.session.telemetrySensor then
    frsky.reset()
    return
  end
  if not (inavsuite.tasks and inavsuite.tasks.telemetry and inavsuite.tasks.msp and inavsuite.tasks.msp.mspQueue) then
    return
  end
  ...
  if fp ~= _lastSlotsFp then
    _lastSlotsFp = fp
    frsky.setFblSensors(inavsuite.session.telemetryConfig or {})
    frsky.reset() -- clears create/rename caches so next frames re-apply rules
  end  

  if inavsuite.app and inavsuite.app.guiIsRunning == false and inavsuite.tasks.msp.mspQueue:isProcessed() then


    -- Only drain frames while there is work to do (create/rename).
    if telemetryActive() and inavsuite.session.telemetrySensor and hasPendingActions() then
      local n = 0
      while telemetryPop() do
        n = n + 1
        if n >= 50 then break end
        if inavsuite.session.mspBusy == true then break end
