...
function render.dirty(box)
    if not inavsuite.session.telemetryState then return false end
...
local function eraseBlackboxAsk()
    ...
    form.openDialog({
        title =  inavsuite.i18n.get("widgets.bbl.erase_dataflash"),
        message = inavsuite.i18n.get("widgets.bbl.erase_dataflash") .. "?",
        buttons = buttons,
        options = TEXT_LEFT
    })
end
...
local function eraseDataflash()
    ...
    progress = form.openProgressDialog(
        inavsuite.i18n.get("app.msg_saving"),
        inavsuite.i18n.get("app.msg_saving_to_fbl")
    )
    progress:value(0)
...
local function ensureCfg(box)
    local theme_version = (inavsuite and inavsuite.theme and inavsuite.theme.version) or 0
...
function render.wakeup(box)
    ...
    -- Extract telemetry
    local totalSize = inavsuite.session.bblSize
    local usedSize  = inavsuite.session.bblUsed
    ...
    if totalSize and usedSize then
        ...
        displayValue = string.format("%." .. cfg.decimals .. "f/%." .. cfg.decimals .. "f %s",
            transformedUsed, transformedTotal, inavsuite.i18n.get("app.modules.fblstatus.megabyte"))
    else
