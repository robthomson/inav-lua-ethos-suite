...
This section provides a high-level view of the Inav Lua Ethos Suite (`inavsuite`) and its major components.
...
1. **Initialization**: `main.lua` sets up `inavsuite.config`, loads or creates user preferences (`inavsuite.preferences`), initializes session variables (`inavsuite.session`), and starts the background task (`inavsuite.tasks`) alongside the UI application (`inavsuite.app`).
2. **Tasks vs. App**:

   * **Tasks**: Handle low-level MSP protocol interactions, scheduling, parsing, and callbacks. Exposed via the `inavsuite.tasks.msp.api` and `inavsuite.tasks.callback` systems.
...
4. **Session & Preferences**:
   ...
   * **Preferences**: Stored in an INI file under `inavsuite.userpreferences`, merged with defaults on startup.
   * **Session**: Runtime state (e.g., `activeProfile`, `apiVersion`, `flightMode`) tracked in `inavsuite.session` to coordinate between tasks and UI.

## 2. Module Creation Guide (`inavsuite/app/modules`)
...
1. **Initialization**:
   ...
   ```lua
   local API = inavsuite.app.Page.apidata.load(apiName)
   app.Page.apidata = {
     api      = API,
     formdata = API.data(),
...
4. **Submission & Callbacks**:
   ...
   * Use `inavsuite.tasks.callback.now()` via `api.scheduleWakeup()` for asynchronous operations.
...
* For custom behavior, modules can invoke `inavsuite.tasks.msp.api.scheduleWakeup()` to schedule periodic reads or writes.

## 3. MSP API Documentation (`inavsuite/tasks/msp/api`)
