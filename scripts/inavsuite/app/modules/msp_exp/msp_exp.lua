...
local total_bytes = inavsuite.preferences.developer.mspexpbytes
local fieldMap = {}
...
local function buildFieldMap()
    fieldMap = {}
    for _, field in ipairs(inavsuite.app.Page.fields or {}) do
...
-- Init API
local apidata = generateMSPAPI(inavsuite.preferences.developer.mspexpbytes)
...
local function postLoad()
    ...
    if inavsuite.app.Page.apidata.receivedBytesCount['EXPERIMENTAL'] == 0 then
    ...
    if total_bytes ~= inavsuite.app.Page.apidata.receivedBytesCount['EXPERIMENTAL'] then

        inavsuite.preferences.developer.mspexpbytes = inavsuite.app.Page.apidata.receivedBytesCount['EXPERIMENTAL']
...
local function wakeup()
    if not enableWakeup or not inavsuite.app.Page or not inavsuite.app.Page.fields then
        return
    end
    periodicSync()
end
