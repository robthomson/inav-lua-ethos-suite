...
def resolve_i18n_tags_in_place(out_dir, lang="en"):
    ...
    json_path = os.path.join(out_dir, "i18n", f"{lang}.json")  # because scripts/inavsuite/** got copied already
    if not os.path.isfile(json_path):
        # Fallback for local build before copy, if needed:
        json_path = os.path.join(config['git_src'], "scripts", "inavsuite", "i18n", f"{lang}.json")
    if not os.path.isfile(json_path):
