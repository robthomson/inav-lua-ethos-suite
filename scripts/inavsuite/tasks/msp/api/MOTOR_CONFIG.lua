...
local function write(suppliedPayload)
    ...
    local message = {
        ...
        payload = suppliedPayload or inavsuite.tasks.msp.api.buildWritePayload(API_NAME, payloadData,MSP_API_STRUCTURE_WRITE, MSP_REBUILD_ON_WRITE), -- we do a true on write as we will force rebuild the payload
        processReply = function(self, buf)
            local completeHandler = handlers.getCompleteHandler()
            if completeHandler then completeHandler(self, buf) end
