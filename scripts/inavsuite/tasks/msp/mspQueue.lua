...
local function LOG_ENABLED_MSP()
    return inavsuite and inavsuite.preferences and inavsuite.preferences.developer and inavsuite.preferences.developer.logmsp
end

local function LOG_ENABLED_MSP_QUEUE()
    return inavsuite and inavsuite.preferences and inavsuite.preferences.developer and inavsuite.preferences.developer.logmspQueue
end
...
function MspQueueController:processQueue()
    ...
    if self:isProcessed() then
        inavsuite.app.triggers.mspBusy = false
        self.mspBusyStart = nil
        return
    end
    ...
    if self.mspBusyStart and (os.clock() - self.mspBusyStart) > mspBusyTimeout then
        --inavsuite.utils.log("MSP busy timeout exceeded. Forcing clear.", "info")
        inavsuite.app.triggers.mspBusy = false
        self.mspBusyStart = nil
        return
    end

    inavsuite.app.triggers.mspBusy = true
    ...
    local lastTimeInterval = inavsuite.tasks.msp.protocol.mspIntervalOveride or 0.5
    ...
    if not system:getVersion().simulation then
        ...
        if not self.currentMessage.simulatorResponse then
            if LOG_ENABLED_MSP() then
                inavsuite.utils.log("No simulator response for command " .. tostring(self.currentMessage.command), "debug")
            end
            self.currentMessage = nil
            self.uuid = nil
            return
