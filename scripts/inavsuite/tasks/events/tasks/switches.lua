...
-- 
-- This function retrieves the user's switch preferences from `inavsuite.preferences.switches`.
...
-- @usage
--   initializeSwitches()
-- @see inavsuite.preferences.switches
-- @see system.getSource
-- @see inavsuite.tasks.telemetry.listSensorAudioUnits
local function initializeSwitches()
    local prefs = inavsuite.preferences.switches
    ...
    switchTable.units = inavsuite.tasks.telemetry.listSensorAudioUnits()
end
...
--   - `inavsuite.tasks.telemetry.getSensorSource(key)`: Retrieves the sensor source for a switch.
...
function switches.wakeup()
    ...
    local mode = (inavsuite and inavsuite.flightmode and inavsuite.flightmode.current) or nil
    ...
    for key, sensor in pairs(switchTable.switches) do
        ...
        if playNow then
            local sensorSrc = inavsuite.tasks.telemetry.getSensorSource(key)
            if sensorSrc then
