...
function utils.resolveModelName(foldername)
    ...
    local iniName = "LOGS:inavsuite/telemetry/" .. foldername .. "/logs.ini"
...
function utils.getLogPath()
    ...
    if inavsuite.app.activeLogDir then
        return string.format("LOGS:/inavsuite/telemetry/%s/", inavsuite.app.activeLogDir)
    end
    return "LOGS:/inavsuite/telemetry/"
end
...
function utils.getLogDir(dirname)
    ...
    if not dirname then
        local defaultDir = "LOGS:/inavsuite/telemetry/" .. inavsuite.session.mcu_id .. "/"
        os.mkdir(defaultDir)
        return defaultDir
    end

    -- Return requested directory
    return "LOGS:/inavsuite/telemetry/" .. dirname .. "/" 
end
