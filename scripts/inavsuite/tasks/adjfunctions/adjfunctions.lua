...
function adjfunc.wakeup()
  local now = os.clock()
  local events = inavsuite.preferences and inavsuite.preferences.events
  if not (events and (events.adj_f or events.adj_v)) then return end
  if (now - initTime) < 5 then return end

  adjfuncAdjValue = inavsuite.tasks.telemetry.getSensor("adj_v")
  adjfuncAdjFunction = inavsuite.tasks.telemetry.getSensor("adj_f")
...
if adjfuncPendingFuncAnnounce and not firstRun and events.adj_f then
          ...
          if wavs then
            local playFile = inavsuite.utils and inavsuite.utils.playFile
            if playFile then
              for i = 1, #wavs do
