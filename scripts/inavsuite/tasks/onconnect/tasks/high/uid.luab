...
function uid.wakeup()
    ...
    if (inavsuite.session.mcu_id == nil and mspCallMade == false)  then


        mspCallMade = true

        local API = inavsuite.tasks.msp.api.load("UID")
        API.setCompleteHandler(function(self, buf)
            ...
            if U_ID_0 and U_ID_1 and U_ID_2 then
                ...
                if uid then
                    inavsuite.utils.log("MCU ID: " .. uid, "info")
                end
                inavsuite.session.mcu_id = uid
            end
...
function uid.reset()
    inavsuite.session.mcu_id = nil
    mspCallMade = false
end

function uid.isComplete()
    if inavsuite.session.mcu_id ~= nil  then
        return true
    end
end
