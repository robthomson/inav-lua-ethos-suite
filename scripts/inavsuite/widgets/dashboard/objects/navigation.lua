...
local renders = inavsuite.widgets.dashboard.renders
local folder = "SCRIPTS:/" .. inavsuite.config.baseDir .. "/widgets/dashboard/objects/navigation/"
...
function wrapper.wakeup(box)
    ...
    if not renders[subtype] then
        ...
        local loader = inavsuite.compiler.loadfile(path)
        if loader then
