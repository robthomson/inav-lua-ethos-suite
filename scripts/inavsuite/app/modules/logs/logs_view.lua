...
local graphPos = {}
graphPos['menu_offset'] = inavsuite.app.radio.logGraphMenuOffset
graphPos['height_offset'] = inavsuite.app.radio.logGraphHeightOffset or 0
graphPos['x_start'] = 0
graphPos['y_start'] = 0 + graphPos['menu_offset']
graphPos['width'] = math.floor(LCD_W * inavsuite.app.radio.logGraphWidthPercentage)
...
local logColumns = inavsuite.tasks.logging.getLogTable()
...
function readNextChunk()
    ...
    if chunk then
        ...
        inavsuite.utils.log("Read " .. #chunk .. " bytes from log file","debug")
    else
        logFileHandle:close()
        logFileHandle = nil
        ...
        inavsuite.utils.log("Read complete, total size: " .. #logDataRaw .. " bytes","debug")
    end
end
...
local function drawKey(name, keyunit, keyminmax, keyfloor, color, minimum, maximum, laneY, laneHeight)
    ...
    lcd.font(inavsuite.app.radio.logKeyFont)
    ...
    lcd.font(inavsuite.app.radio.logKeyFontSmall)
    ...
    if inavsuite.app.radio.logShowAvg == true then
...
local function drawCurrentIndex(points, position, totalPoints, keyindex, keyunit, keyfloor, name, color, laneY, laneHeight, laneNumber, totalLanes)
    ...
    local sliderPadding = inavsuite.app.radio.logSliderPaddingLeft
...
local function openPage(pidx, title, script, logfile, displaymode,dirname)
    ...
    inavsuite.app.ui.fieldHeader("Logs / ".. name .. " / " .. extractShortTimestamp(logfile))
    activeLogFile = logfile

    local filePath

    if inavsuite.app.activeLogDir then
        filePath = utils.getLogDir(inavsuite.app.activeLogDir) .. "/" .. logfile
    else
    ...
    inavsuite.app.formFields[1] = form.addSliderField(nil, posField, 0, 100, function()
    ...
    inavsuite.app.formFields[2] = form.addButton(line, posField, {
        text = "-",
        icon = nil,
        options = FONT_M,
        press = function()
            if zoomLevel > 1 then
                ...
                lcd.invalidate()
                inavsuite.app.formFields[2]:enable(true)
                inavsuite.app.formFields[3]:enable(true)
            end    
            if zoomLevel == 1 then
                inavsuite.app.formFields[2]:enable(false)
                inavsuite.app.formFields[3]:focus()   
            end
        end
    })
    -- disable on start
    ...
    inavsuite.app.formFields[3] = form.addButton(line, posField, {
        text = "+",
        icon = nil,
        options = FONT_M,
        press = function()
            ...
            if zoomLevel == zoomCount then
                ...
                inavsuite.app.formFields[2]:focus()   
            end    
        end
    })
    
    inavsuite.app.formFields[1]:step(1)
...
local function event(event, category, value, x, y)
    if  value == 35 then
        inavsuite.app.ui.openPage(inavsuite.app.lastIdx, inavsuite.app.lastTitle, "logs/logs_logs.lua")
        return true
    end
    return false
end
...
local function wakeup()
    ...
    if logDataRawReadComplete and not processedLogData then
        ...
        if not carriedOver then
            ...
            inavsuite.app.formNavigationFields['menu']:focus(true)
        ...
        if currentDataIndex >= #logColumns then
            ...
            local btnMinus = inavsuite.app.formFields[2]
            local btnPlus  = inavsuite.app.formFields[3]
            if zoomCount <= 1 then
