...
elrs.setFblSensors(inavsuite.session.telemetryConfig)
...
function elrs.crossfirePop()
  ...
  if (CRSF_PAUSE_TELEMETRY == true or inavsuite.session.mspBusy == true or not telemetryActive()) then
...
function elrs.wakeup()
  ...
  if fp ~= _lastSlotsFp then
    _lastSlotsFp = fp
    elrs.setFblSensors(inavsuite.session.telemetryConfig or {})
    resetSensors() -- clears local caches so new ones get created on demand
  end

  if telemetryActive() and inavsuite.session.telemetrySensor then
    if not inavsuite.session.mspBusy then 
      local n = 0
      while elrs.crossfirePop() do
        n = n + 1
        if n >= 50 then break end
        if CRSF_PAUSE_TELEMETRY == true or inavsuite.session.mspBusy == true then break end
      end
    end  
    refreshStaleSensors()
  else
