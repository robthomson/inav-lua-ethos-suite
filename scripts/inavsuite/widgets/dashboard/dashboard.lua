...
local compile = inavsuite.compiler.loadfile

local baseDir = inavsuite.config.baseDir
local preferences = inavsuite.config.preferences
...
dashboard.flightmode = inavsuite.flightmode.current or "preflight"
...
-- ===== Repaint governor ==== ------
dashboard._minPaintInterval   = 0.033   -- 50ms ≈ 20 FPS; tune between 0.033–0.1
...
function dashboard.computeOverlayMessage()
    ...
    if inavsuite.session.apiVersion and inavsuite.session.rfVersion and not inavsuite.session.isConnectedLow and state ~= "postflight" then
        if system.getVersion().simulation == true then
            return pad .. "SIM " .. inavsuite.session.apiVersion .. pad
        else
            return pad .. "RF" .. inavsuite.session.rfVersion .. pad
        end
    end

    -- 4) LAST: generic waiting message (don’t let it mask actionable errors)
    if not inavsuite.session.isConnectedHigh and state ~= "postflight" then
...
function dashboard.renderLayout(widget, config)
    ...
    if layout.showgrid or inavsuite.preferences.developer.overlaygrid then
    ...
    if layout.showstats or inavsuite.preferences.developer.overlaystats then
        local headerOffset = (isFullScreen and headerLayout and headerLayout.height) or 0

        local cpuUsage = (inavsuite.performance and inavsuite.performance.cpuload) or 0
        local loopMs   = (inavsuite.performance and inavsuite.performance.loop_ms)   or 0
        local budgetMs = (inavsuite.performance and inavsuite.performance.budget_ms) or 50  -- 20 Hz -> 50ms
        local tickMs   = (inavsuite.performance and inavsuite.performance.tick_ms)   -- optional, if you published it
        local headroomPct = math.max(0, 100 - (cpuUsage or 0))

        local ramFreeKB        = (inavsuite.performance and inavsuite.performance.luaRamKB)        or 0
        local ramUsedGC_KB     = (inavsuite.performance and inavsuite.performance.usedram)         or 0
        local sysRamFreeKB     = (inavsuite.performance and inavsuite.performance.ramKB)           or 0
        local bitmapRamFreeKB  = (inavsuite.performance and inavsuite.performance.luaBitmapsRamKB) or 0
        local mainStackKB      = (inavsuite.performance and inavsuite.performance.mainStackKB)     or 0
        ...
        local function fmtPct(n) return inavsuite.utils.round(n or 0, 0) end
        ...
        local schedRows = {
            ...
            { "LOAD (100ms window)", fmtPct(inavsuite.performance.cpuload_window100 or 0), "%" },
...
local function getThemeForState(state)
    local prefs = inavsuite.session.modelPreferences and inavsuite.session.modelPreferences.dashboard
    local fallback = inavsuite.preferences.dashboard
...
local function getThemeForState(state)
    local modelPrefs = inavsuite.session.modelPreferences and inavsuite.session.modelPreferences.dashboard
    local userPrefs = inavsuite.preferences.dashboard
    local val = nil
    if modelPrefs then
...
function dashboard.create()
    ...
    if not dashboard.utils then
        dashboard.utils = assert(compile("SCRIPTS:/" .. inavsuite.config.baseDir .. "/widgets/dashboard/lib/utils.lua"))()
    end
    if not dashboard.loaders then
        dashboard.loaders = assert(compile("SCRIPTS:/" .. inavsuite.config.baseDir .. "/widgets/dashboard/lib/loaders.lua"))()
    end

    -- 2) ensure user theme dir exists
    os.mkdir("SCRIPTS:/" .. inavsuite.config.preferences .. "/dashboard/")
...
function dashboard.event(widget, category, value, x, y)
    ...
    if state == "postflight" and category == EVT_KEY and value == 131 then
        inavsuite.widgets.dashboard.flightmode = "preflight"
...
function dashboard.wakeup(widget)
    ...
    if inavsuite.session and inavsuite.session.mspBusy and not (inavsuite.session and inavsuite.session.isConnected) then return end

    -- Quick exit if not visible or running admin app
    local now = os.clock()
    local visible = lcd.isVisible()
    local admin = inavsuite.app and inavsuite.app.guiIsRunning 
    ...
    objectProfiler = inavsuite.preferences and inavsuite.preferences.developer and inavsuite.preferences.developer.logobjprof
    ...
    if firstWakeupCustomTheme and
        inavsuite.session.mcu_id and
        inavsuite.session.modelPreferences and
        inavsuite.session.modelPreferences.dashboard then

        local modelPrefs = inavsuite.session.modelPreferences.dashboard
        local currentPrefs = inavsuite.preferences.dashboard
    ...
    local currentFlightMode = inavsuite.flightmode.current or "preflight"
                ...
                if utils.dir_exists(basePath, folder) then
                    ...
                    if chunk then
                        ...
                        if ok and initTable and type(initTable.name) == "string" then
                            if not initTable.developer or inavsuite.preferences.developer.devtools == true then
                                num = num + 1
                                themes[num] = {
                                    name = initTable.name,
...
function dashboard.getPreference(key)
    if not inavsuite.session.modelPreferences or not dashboard.currentWidgetPath then return nil end

    if not inavsuite.app.guiIsRunning then
        return inavsuite.ini.getvalue(inavsuite.session.modelPreferences, dashboard.currentWidgetPath, key)
    else
        return inavsuite.ini.getvalue(inavsuite.session.modelPreferences, inavsuite.app.dashboardEditingTheme, key)
    end
end
...
function dashboard.savePreference(key, value)
    if not inavsuite.session.modelPreferences or not inavsuite.session.modelPreferencesFile or not dashboard.currentWidgetPath then
        return false
    end
    if not inavsuite.app.guiIsRunning then
        inavsuite.ini.setvalue(inavsuite.session.modelPreferences, dashboard.currentWidgetPath, key, value)
        return inavsuite.ini.save_ini_file(inavsuite.session.modelPreferencesFile, inavsuite.session.modelPreferences)
    else
        inavsuite.ini.setvalue(inavsuite.session.modelPreferences, inavsuite.app.dashboardEditingTheme, key, value)
        return inavsuite.ini.save_ini_file(inavsuite.session.modelPreferencesFile, inavsuite.session.modelPreferences)
    end
end
