...
function elrs.setFblSensors(list)
  ...
  -- We need sid to map inav id -> sidElrs (load, then free)
  local sidList = getSidList()
  if sidList then
...
function elrs.crossfirePop()
  ...
  if (CRSF_PAUSE_TELEMETRY == true or inavsuite.app.triggers.mspBusy == true or not telemetryActive()) then
...
function elrs.wakeup()
  ...
  if telemetryActive() and inavsuite.session.telemetrySensor then
    ...
    while elrs.crossfirePop() do
      ...
      if CRSF_PAUSE_TELEMETRY == true or inavsuite.app.triggers.mspBusy == true then break end
    end
    refreshStaleSensors()
  else
    resetSensors()
