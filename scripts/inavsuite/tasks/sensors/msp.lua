...
local function createOrUpdateSensor(appId, fieldMeta, value)
    ...
    local currentModule = inavsuite.session.telemetrySensor
                        and inavsuite.session.telemetrySensor:module()
...
local function updateSessionField(meta, value)
    if not meta.sessionname or type(inavsuite.session) ~= "table" then return end
    local t = inavsuite.session
...
function msp.wakeup()
    ...
    if inavsuite.flightmode and inavsuite.flightmode.current ~= "inflight" then
    ...
    if inavsuite.session.resetMSPSensors then
        sensorCache   = {}
        negativeCache = {}
        lastValue     = {}
        lastPush      = {}
        inavsuite.session.resetMSPSensors = false
    end
    ...
    local isAdmin = inavsuite.app.guiIsRunning
