...
function postLoad()
    ...
    if inavsuite.app.Page.apidata and inavsuite.app.Page.apidata.other  and inavsuite.app.Page.apidata.other['ESC_PARAMETERS_HW5'] then
        local version
        if inavsuite.session.escDetails and inavsuite.session.escDetails.version then
            version = inavsuite.session.escDetails.version
        else
            version = "default"
        end

        if inavsuite.app.Page.apidata.other['ESC_PARAMETERS_HW5'][version] then
            local newVoltage = inavsuite.app.Page.apidata.other['ESC_PARAMETERS_HW5'][version]

            local voltageTable = inavsuite.app.utils.convertPageValueTable(newVoltage, -1)   
    
            -- we target field 3 as we know it is bec_voltage
            inavsuite.app.formFields[3]:values(voltageTable)
        end
...
local function onNavMenu(self)
    inavsuite.app.triggers.escToolEnableButtons = true
    inavsuite.app.ui.openPage(pidx, folder, "esc_tools/esc_tool.lua")
end
...
return {
    ...
    headerLine = inavsuite.escHeaderLineText
}
