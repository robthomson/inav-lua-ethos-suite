...
local compiler = inavsuite.compiler 
...
function utils.session()
    ...
    --inavsuite.utils.log("Session initialized", "info")
end


--- Checks if the RX map is ready by verifying the presence of required channel mappings.
-- The function returns true if the `inavsuite.session.rx.map` table exists and at least one of the following fields is present:
...
function utils.rxmapReady()
    if inavsuite.session.rx and inavsuite.session.rx.map and (inavsuite.session.rx.map.collective or inavsuite.session.rx.map.elevator or inavsuite.session.rx.map.throttle or inavsuite.session.rx.map.rudder) then
        return true
    end
    return false
end
...
function utils.inFlight()
    if inavsuite.flightmode.current == "inflight" then
        return true
    end
    return false
end
...
function utils.msp_version_array_to_indexed()
    local arr = {}
    local tbl = inavsuite.config.supportedMspApiVersion or {"12.06", "12.07", "12.08"}
...
function utils.getGovernorState(value)
    ...
    if not inavsuite.tasks.telemetry then
...
function utils.log(msg, level)
    if inavsuite.tasks and inavsuite.tasks.logger then
        inavsuite.tasks.logger.add(msg, level or "debug")
    end
end
...
function utils.logMsp(cmd, rwState, buf, err)
    if inavsuite.preferences.developer.logmsp then
        ...
        if err then
            inavsuite.utils.log("Error: " .. err, "info")
        end
    end
end
...
function utils.reportMemoryUsage(location)
    ...
    local cpuInfo = inavsuite.session and inavsuite.session.cpuload or 0
    ...
    inavsuite.utils.log(string.format("[%s] CPU Load: %.2f ", location, inavsuite.utils.round(cpuInfo,0)) .. "%", "info")
    inavsuite.utils.log(string.format("[%s] Main stack available: %.2f KB", location, mainStackKB), "info")
    inavsuite.utils.log(string.format("[%s] System RAM available: %.2f KB", location, ramKB), "info")
    inavsuite.utils.log(string.format("[%s] Lua RAM available: %.2f KB", location, luaRamKB), "info")
    inavsuite.utils.log(string.format("[%s] Lua Bitmap RAM available: %.2f KB", location, luaBitmapsRamKB), "info")
end
...
function utils.apiVersionCompare(op, req)
    ...
    local a, b = parts(inavsuite.session.apiVersion or 12.06), parts(req)
