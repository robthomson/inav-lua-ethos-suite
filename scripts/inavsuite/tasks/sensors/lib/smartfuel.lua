...
local lastMode = inavsuite.flightmode.current or "preflight" -- Last flight mode to detect changes.
...
local function smartFuelCalc()
    ...
    if currentMode ~= lastMode then
        inavsuite.utils.log("Flight mode changed – resetting voltage & fuel state", "info")
   ...
   if stabilizeNotBefore and now < stabilizeNotBefore then
    ...
    local isDisarmed  = (inavsuite and inavsuite.session and inavsuite.session.isArmed == false)
    local isPreflight = (inavsuite and inavsuite.flightmode and inavsuite.flightmode.current == "preflight")
