...
local compiler = inavsuite.compiler.loadfile
...
function tasks.isTaskActive(name)
    for _, t in ipairs(tasksList) do
        if t.name == name then
            ...
            if name == "msp" then
                return inavsuite.app.triggers.mspBusy
...
function tasks.initialize()
    ...
    if io.open(cachePath, "r") then
        local ok, cached = pcall(inavsuite.compiler.dofile, cachePath)
...
function tasks.telemetryCheckScheduler()
    ...
    if system.getVersion().simulation and inavsuite.simevent.telemetry_state == false then
...
function tasks.active()
    ...
    tasks.wasOn = age >= 2
    if inavsuite.app.triggers.mspBusy or age <= 2 then return true end
...
local function canRunTask(task, now)
    ...
    local ok =
        linkOK
        and connOK
        and (priorityTask or od >= 0 or not inavsuite.app.triggers.mspBusy)
...
function tasks.wakeup()
  ...
  inavsuite.session.cpuload = math.min(100, math.max(0, cpu_avg * 100))
  ...
  last_wakeup_start = now
    ...
    if (now2 - last_mem_t) >= MEM_PERIOD then
        ...
        inavsuite.session.freeram   = mem_avg_kb          -- KB (ema)
        inavsuite.session.luaUsedKb = collectgarbage and collectgarbage("count") or nil
        inavsuite.session.memSource = "system"
        else
        ...
        inavsuite.session.luaUsedKb = used_kb
        -- keep last known free if we had one; otherwise mark as nil
        inavsuite.session.freeram   = mem_avg_kb          -- may be nil if never known
        inavsuite.session.memSource = "lua"
        end
    end
    end
...
-- After updating all tasks, invokes the `inavsuite.utils.session()` function.
function tasks.setTelemetryTypeChanged()
    for _, task in ipairs(tasksList) do
        if tasks[task.name].setTelemetryTypeChanged then
            --inavsuite.utils.log("Notifying task [" .. task.name .. "] of telemetry type change", "info")
