...
-- This function returns true if the `inavsuite` table, its `session` field,
...
function utils.isModelPrefsReady()
    return inavsuite and inavsuite.session and inavsuite.session.modelPreferences
end
...
function utils.getFontListsForResolution()
    ...
    if not radios[resolution] then
        inavsuite.utils.log("Unsupported resolution: " .. resolution .. ". Using default fonts.","info")
...
function utils.box(
    ...
    if image then
        ...
        if type(image) == "string" and inavsuite and inavsuite.utils and inavsuite.utils.loadImage then
            imageCache = imageCache or {}
            local cacheKey = image or "default_image"
            bitmapPtr = imageCache[cacheKey]
            if not bitmapPtr then
                bitmapPtr = inavsuite.utils.loadImage(image, nil, "widgets/dashboard/gfx/logo.png")
