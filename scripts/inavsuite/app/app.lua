...
local function rebootFc()
  ...
  inavsuite.tasks.msp.mspQueue:add({
...
function app.settingsSaved()
  if app.Page and app.Page.eepromWrite then
    if app.pageState ~= app.pageStatus.eepromWrite then
      ...
      app.triggers.closeSave = true
      if inavsuite.session.isArmed then
...
local function saveSettings()
  ...
  inavsuite.app.Page.apidata.apiState.isProcessing = true
  ...
  for apiID, apiNAME in ipairs(apiList) do
    ...
    local API = inavsuite.tasks.msp.api.load(apiNAME)
...
local function requestPage()
  ...
  local function processNextAPI()
    ...
    if not apiKey then
      ...
      inavsuite.tasks.callback.inSeconds(backoff + jitter, processNextAPI)
      return
    end

    local API = inavsuite.tasks.msp.api.load(v)
    ...
    local function handleTimeout()
      ...
      if retryCount < 3 then
        ...
        inavsuite.tasks.callback.inSeconds(0.25, processNextAPI)
      else
    ...
    inavsuite.tasks.callback.inSeconds(2, handleTimeout)
    ...
    API.setCompleteHandler(function(self, buf)
      ...
      state.currentIndex = state.currentIndex + 1
      inavsuite.tasks.callback.inSeconds(0.5, processNextAPI)
    end)
...
function app.updateTelemetryState()
  if system:getVersion().simulation ~= true then
    if not inavsuite.session.telemetrySensor then
...
app._uiTasks = {
  ...
  function()
    if not (app.Page and (app.Page.refreshOnProfileChange or app.Page.refreshOnRateChange or app.Page.refreshFullOnProfileChange or app.Page.refreshFullOnRateChange)
           ...
           and inavsuite.tasks.msp.mspQueue:isProcessed()) then return end
    local now = os.clock();
    local interval = (inavsuite.tasks.telemetry.getSensorSource("pid_profile") and inavsuite.tasks.telemetry.getSensorSource("rate_profile"))
                     and 0.1 or 1.5
    if (now - (app.profileCheckScheduler or 0)) >= interval then
      app.profileCheckScheduler = now
      app.utils.getCurrentProfile()
      if inavsuite.session.activeProfileLast and app.Page.refreshOnProfileChange and
         inavsuite.session.activeProfile ~= inavsuite.session.activeProfileLast then
        app.triggers.reload = not app.Page.refreshFullOnProfileChange
        app.triggers.reloadFull = app.Page.refreshFullOnProfileChange
        return
      end
      if inavsuite.session.activeRateProfileLast and app.Page.refreshOnRateChange and
         inavsuite.session.activeRateProfile ~= inavsuite.session.activeRateProfileLast then
  ...
  function()
    ...
    if not app.dialogs.progressDisplay then
      ...
      app.dialogs.progressCounter = 0
      local key = (inavsuite.utils.apiVersionCompare(">=", "12.08") and "app.msg_please_disarm_to_save_warning" or "app.msg_please_disarm_to_save")
  ...
  function()
    ...
    if app.uiState == app.uiStatus.mainMenu then
      invalidatePages()
    elseif app.triggers.isReady and (inavsuite.tasks and inavsuite.tasks.msp and inavsuite.tasks.msp.mspQueue:isProcessed())
...
-- Close and cleanup
function app.close()
  inavsuite.utils.reportMemoryUsage("closing application: start")
  ...
  inavsuite.session.activeProfileLast = nil
  inavsuite.session.activeProfile = nil
  inavsuite.session.activeRateProfile = nil
  inavsuite.session.activeRateProfileLast = nil
  inavsuite.session.activeRateTable = nil

  collectgarbage()
  invalidatePages()

  inavsuite.utils.reportMemoryUsage("closing application: end")
  system.exit()
  return true
end
