...
function utils.getRSSI()
    if inavsuite.simevent.rflink == 1 then
        return 0
    end

    if inavsuite.app.offlineMode == true then
        return 100
    end

    if inavsuite.session.telemetryState then
        return 100
    else
        return 0
    end
...
function utils.getFieldValue(f)
    ...
    if f.decimals then
        v = inavsuite.utils.round(v * inavsuite.app.utils.decimalInc(f.decimals), 2)
    end
...
--[[
    ...
    @param f (table) Field to save to. Optional keys:
        ...
        - postEdit (func)   Called as postEdit(inavsuite.app.Page) after save.
...
function utils.saveFieldValue(f, value)
    if value then
        ...
        if f.decimals then
            f.value = value / inavsuite.app.utils.decimalInc(f.decimals)
        else
            f.value = value
        end
...
function utils.scaleValue(value, f)
    ...
    local v = value * inavsuite.app.utils.decimalInc(f.decimals)
    if f.scale then v = v / f.scale end
    return inavsuite.utils.round(v)
end
...
function utils.getInlinePositions(f, lPage)
    ...
    local inline_size = utils.getInlineSize(f.label, lPage) * inavsuite.app.radio.inlinesize_mult
    ...
    local eH      = inavsuite.app.radio.navbuttonHeight
    local eY      = inavsuite.app.radio.linePaddingTop
...
function utils.getCurrentProfile()
    local pidProfile  = inavsuite.tasks.telemetry.getSensor("pid_profile")
    local rateProfile = inavsuite.tasks.telemetry.getSensor("rate_profile")

    if (pidProfile ~= nil and rateProfile ~= nil) then
        inavsuite.session.activeProfileLast = inavsuite.session.activeProfile
        local p = pidProfile
        if p ~= nil then
            inavsuite.session.activeProfile = math.floor(p)
        else
            inavsuite.session.activeProfile = nil
        end

        inavsuite.session.activeRateProfileLast = inavsuite.session.activeRateProfile
        local r = rateProfile
        if r ~= nil then
            inavsuite.session.activeRateProfile = math.floor(r)
        else
