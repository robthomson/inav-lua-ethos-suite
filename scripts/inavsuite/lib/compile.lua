--[[
 * Inav Project - Enhanced Script Compilation and Caching
 *
...
-- Configuration: expects inavsuite.config to be globally available
local logTimings = true
if inavsuite and inavsuite.config then
  if type(inavsuite.preferences.developer.compilerTiming) == "boolean" then
    logTimings = inavsuite.preferences.developer.compilerTiming or false
  end
end
...
local function LRUCache()
  ...
  local function currentLuaRamKB()
    -- Prefer inavsuite session metrics (already in KB)
    if inavsuite and inavsuite.performance then
      local osinfo = inavsuite.performance
...
function compile.wakeup()
  ...
  if #compile._queue > 0 then
    ...
    if inavsuite and inavsuite.utils and log then
      if ok then
        inavsuite.utils.log("Deferred-compiled (throttled): " .. entry.script, "info")
      else
        inavsuite.utils.log("Deferred-compile error: " .. tostring(err), "debug")
      end
    end
  end
end
...
function compile.loadfile(script, opts)
  ...
  if loader then
    ...
    if not inavsuite.preferences.developer.compile then
...
function compile.clearAll()
  ...
  lru_cache.order  = {}
  lru_cache.pinned = {}
  if inavsuite and inavsuite.utils then
    inavsuite.utils.log("Cleared all cached scripts (" .. total .. " evicted)", "info")
  end
end
