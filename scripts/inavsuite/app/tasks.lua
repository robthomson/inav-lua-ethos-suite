...
local function profileRateChangeDetection()
    ...
    if not (
        ...
        inavsuite.tasks.msp.mspQueue:isProcessed()
    ) then
        return
    end
    ...
    local now = os.clock()
    local interval = (
        inavsuite.tasks.telemetry.getSensorSource("pid_profile") and
        inavsuite.tasks.telemetry.getSensorSource("rate_profile")
    ) and 0.1 or 1.5
...
local function armedSaveWarning()
    ...
    if not app.dialogs.progressDisplay then
        ...
        local key =
            (inavsuite.utils.apiVersionCompare(">=", "12.08")
